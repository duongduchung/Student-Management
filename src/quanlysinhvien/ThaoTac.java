...
public class ThaoTac{ 
    ...
    public static void findID(SinhVien[] sinhVien, int n) throws Exception 
    { 
        Scanner input = new Scanner(System.in); 
        ...
        try 
        { 
            System.out.print("\nBạn nhập vào mã số cần tìm kiếm: "); 
            ID = input.nextLine(); 

            removeSpace(ID, true); 
            location = checkIDExists(sinhVien, ID, n);//lấy vị trí của học sinh có ID bằng ID nhập vào //lấy số lượng điểm trong danh sách bằng với điểm được nhập vào. 

            if(location >= 0) 
            { 
                printTile(); 

                //in ra thông tin của sinh viên có mã số cần tìm
               
                    System.out.printf("\n| %-10d |", location); 
                    sinhVien[location].displaySinhVien();//in thông tin của  sinh viên 
                    System.out.printf("| %-10.2f |", sum(sinhVien[location])); 
                    printFormat();//in ra các line 
                 
            } 
            else 
            { 
                System.out.printf("Không có sinh viên nào có mã số = \"%-4.2f\"", ID); 
            } 
        } 
        catch(Exception ex) 
        { 
